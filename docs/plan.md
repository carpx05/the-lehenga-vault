...
| 2026-09-15 | 6, 8 | PostgREST Schema Alignment & Self-Healing Upsert Loop (`supabase.ts`) | Removed unmapped `price` field from cloud payload to match standard table schema (`buy_price`, `current_price`, `rent`), and added dynamic missing-column interceptor loop for multi-schema resilience. |
